...
private:
  void read_message()
  {
    ...
    boost::asio::async_read_until(socket_, buffer_, "\r\n",
        [this, self](boost::system::error_code ec, std::size_t length)
        {
          if (!ec)
          {
            ...
            for (int i=0; i<3; ++i) {
              msg_definition = msg_definition + message[i];
            }
    ...
    std::vector<std::string*> parts = {&msg, &id, &data, &leitura};
    
    for (size_t i = 0; i < message.length(); ++i) {
        if (message[i] != '|') {
            (*parts[contador_separadores]) += message[i];
        } else {
    ...
    if (msg_definition == "GET") {
        ...
        std::vector<std::string*> parts = {&msg, &id, &numb_reg};
